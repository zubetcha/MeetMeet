import React, {useCallback, useEffect, useState, Children, useRef} from 'react'
...
export const MultiSelect=({
  ...
  initialData
    ...
    useEffect(()=>{
      if(checkedItem && checkedItem.length>0 && checkedItem.length !== values.length){
   ...
   const onChangeTotal=(e: React.ChangeEvent<HTMLInputElement>)=>{
    ...
    useOutsideAlerter(wrapperRef, onInActiveToggle);
        ...
        <div 
          ...
          ref={wrapperRef}>
                    ...
                    onClick={onActiveToggle}>
                      ...
                      <div className={classes.checkedItemResultBox}>
                        <div style={{width:"100%", textOverflow:"ellipsis", overflow:"hidden", whiteSpace:"nowrap"}}>
                          {checkedItemResult && checkedItemResult.map((ck,i)=>ck.name).join(', ')}
                        </div>
                      </div>
                    }
                </div>
            <ul 
              ...
              style={{boxShadow: isboxShadow? "0px 4px 8px rgba(0, 0, 0, 0.16)" : "0"}}>
              <MultiOption
              ...
              <div style={{width:"100%", marginTop:"8px", display:"flex", columnGap:"8px"}}>
                <div style={{boxShadow: isboxShadow? "0px 4px 8px rgba(0, 0, 0, 0.16)" : "0"}}>
                  <Button text="창 닫기" style="line" width={"fit-content"} onClick={onInActiveToggle}></Button>
                </div>
                <div style={{boxShadow: isboxShadow? "0px 4px 8px rgba(0, 0, 0, 0.16)" : "0", width:"100%"}}>
                  <Button text="선택 완료" style="solid" width={"100%"} height={"100%"} onClick={onClickConfirmBtn}></Button>
                </div>
              </div>  
            </div>
          </div>
   ...
   const onChange=(e: React.ChangeEvent<HTMLInputElement>)=>{
    ...
    return(
        ...
        <li 
            ...
            onKeyDown={onKeyDown}
            tabIndex={i}
            className={classNames(classes.select_item, checked && item.name!=="전체 선택"? classes.selected:'' )}
